...
const Sidebar: React.FC<SideProps> = ({active}) => {
  ...
  return (
    <Nav className="flex-column bg-[ #490057] text-white vh-100 p-3 sticky-top" style={{ minWidth: '200px', backgroundColor: ' #490057' }}>
      ...
      <Nav.Link as={NavLink} to={'/profile'} className={`${location.pathname === '/profile' ? 'text-warning' : 'text-white'}`} style={{width: '100%', display: 'flex', alignItems:'center', marginLeft: 'auto'}}><FaUserIcon className="me-2" /> Profile</Nav.Link>
      <Nav.Link as={NavLink} to={'/messages'} className={`${location.pathname === '/messages' ? 'text-warning' : 'text-white'}`} style={{width: '100%', display: 'flex', alignItems:'center', marginLeft: 'auto'}}><FaEnvelopeIcon className="me-2" /> Messages</Nav.Link>
