...
const About: React.FC<UserContextProps> = ({authUser}) => {
  return (
    ...
    <Card.Body className='text-left'>
              <p className='gap-4 justify-content-between d-flex align-items-center'><strong>Gender:</strong> {authUser?.gender || 'Male'}</p>
      ...
      <p className='gap-4 justify-content-between d-flex align-items-center'><strong>Email:</strong> { authUser?.email}</p>
      <p className='gap-4 justify-content-between d-flex align-items-center'><strong>Phone:</strong> { authUser?.phone}</p>
    </Card.Body>
  </Card>
  )
}
