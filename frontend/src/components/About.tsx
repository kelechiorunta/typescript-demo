...
const About: React.FC<AboutContextProps> = ({ authUser, currentUser }) => {
  ...
  const formatBirthday = (birthdate: any) => {
    const birthdayDate = parseISO(birthdate);
    const formattedBirthday = format(birthdayDate, 'MMMM dd, yyyy');
  ...
  return (
    ...
    <Card.Body className='text-left'>
      ...
      <p className='gap-4 justify-content-between d-flex align-items-center'><strong className='d-flex align-items-center'><FaBirthdayIcon className="me-2" />Birthday:</strong>{formatBirthday(currentUser?.birthday) || 'June 26, 1980'}</p>
